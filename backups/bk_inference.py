...
#
# This script runs a real LLM agent (via OpenAI API) against all three tasks
# in our environment and prints the baseline scores.
#
# The hackathon requires this script to:
#   1. Use the OpenAI API client (reads OPENAI_API_KEY from env vars)
#   2. Run the agent against all 3 tasks
#   3. Produce a reproducible baseline score
#
# Usage:
...
def main():
    ...
    print(f"EmailTriageEnv Baseline Inference")
    ...
    # Use the sync wrapper so we don't need asyncio boilerplate
    with EmailTriageEnv(base_url=ENV_BASE_URL).sync() as env_client:
        for task_id in TASKS:
            ...
            for ep in range(EPISODES_PER_TASK):
                ...
                # Normalize by inbox size (5 emails) so score is per-email
                normalized = total_reward / 5.0
                episode_rewards.append(normalized)
                print(f"  → Episode total: {total_reward:.3f} | Normalized: {normalized:.3f}")

            avg = sum(episode_rewards) / len(episode_rewards)
            all_scores[task_id] = round(avg, 3)
            print(f"\n  Average score for '{task_id}': {avg:.3f}")

    print("\n" + "=" * 60)
    print("BASELINE SCORES (copy these into openenv.yaml):")
